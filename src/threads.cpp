...
#include <QObject>
    ...
    void ReceiveThread::testRun() {
        ...
        for (int i=0; i<10; i++){
            qDebug() << "Hardly Working" << QString::number(i) << QThread::currentThread();
            QThread::currentThread()->msleep(500);
        }
        qDebug() << "Finished at 2am" << QThread::currentThread();
        this->deleteLater();    //This apparently deletes the otherwise unmanaged point created by: "ReceiveThread *recThread = new ReceiveThread()" in main.cpp
    }
    ...
    void SendThread::testRun() {
        ...
        for (int i=0; i<10; i++){
            qDebug() << "Me three" << QString::number(i) << QThread::currentThread();
            QThread::currentThread()->msleep(500);
        }
